...
public class MediagroupDAO {
	...
	/*
	1.JdbcTemplate클래스
	2.기본 자바JDBC를 좀 더 편리하게 사용할 수 있다.
	3.JdbcTemplate = DriverManager + Connection + Statement + ResultSet 기능을 해준다.
	4.JDBC를 위한 툴. JDBC프로그래밍에 특화되어 있다.
	● @Autowired
	  -> Spring이 필요시 자동으로 객체를 생성하여 필드(Instance variable, 객체 변수)에 할당함
	● @Repository
	  -> DAO를 스프링에 인식시키기 위해서 주로 사용하며, 해당 클래스를 객체로 만들어 준다
